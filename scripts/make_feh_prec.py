...
rmag = 22.5 - 2.5 * np.log10(FM_T['FLUX_R'])
...
fit_sub = ((RV_T['RR_SPECTYPE'] == 'STAR') & (RV_T['RVS_WARN'] == 0) &
           ...
           (RV_T['VSINI'] < 30)
           & betw(RV_T['TEFF'], 4500, 7000)
           & betw(SC_T['TSNR2_LRG'] * 12.15, 180, 220) & betw(rmag, 15.5, 19))
coeffs = fitter(rmag[fit_sub], RV_T['FEH_ERR'][fit_sub], RV_T['FEH'][fit_sub])
...
for i in range(2):
    ...
    if i == 0:
        plt.ylabel(r'$\log_{10} [\sigma_{FEH}/(1 {\rm km/s})]$ ')
    else:
