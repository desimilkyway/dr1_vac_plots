...
frac = 0.1  # completeness in desi DR1
frac2 = 0.3  # in DESI DR2
# frac = 0.2  # in dr1
nsamp = 1e7
...
shift = 2.5 * np.log(frac / frac2)  # dr1 vs dr2
plot(A1,
     B1 - shift,
     xlog=True,
     xtitle='Distance [kpc]',
     ...
     label='[Fe/H]=$-2.5$',
     color='blue')
oplot(A2, B2 - shift, label='[Fe/H]=$-2.0$', color='green')
oplot(A3, B3 - shift, label='[Fe/H]=$-1.5$', color='red')
oplot(A2, B2, label='DESI DR2', color='grey')
plt.legend()
plt.tight_layout()
plt.fill_between(A1,
